...
const ACTION_BUTTONS = {
  ...
  payment_pending: [
    { action: 'verify_payment', label: '💰 تأكيد الدفع', variant: 'primary', requiresReceipt: true },
  ],
  payment_verification: [
    { action: 'approve', label: '✅ اعتماد الطلب', variant: 'success' },
    { action: 'request_correction', label: '📝 طلب تعديل', variant: 'warning' },
  ],
  approved: [
    { action: 'submit_official', label: '🌐 إدخال رقم التأكيد', variant: 'primary', requiresConfirmation: true },
  ],
  submitted: [
    { action: 'mark_completed', label: '✅ تأكيد الإكتمال', variant: 'success' },
  ],
}

const ALWAYS_ACTIONS = [
  { action: 'request_correction', label: '📝 طلب تعديل', variant: 'warning' },
  { action: 'cancel', label: '❌ إلغاء الطلب', variant: 'error' },
]
...
export default function OrderReview() {
  ...
  const handleAction = async (action) => {
    ...
    if (action === 'verify_payment' && !receiptId) {
      toast.error('الرجاء إدخال معرف الإيصال')
      return
    }

    setActionLoading(true)
    try {
      const body = { action, notes: notes || undefined }
      if (action === 'submit_official') body.confirmation_number = confirmationNumber
      if (action === 'verify_payment') body.receipt_id = receiptId
  ...
  if (!order) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-navy-400">
        ...
        <p>لا يمكن العثور على الطلب المطلوب، ربما تم حذفه أو أن الرابط غير صحيح.</p>
        <Link to="/dashboard/orders" className="mt-6 px-6 py-2 bg-primary-50 text-primary-600 font-bold rounded-xl hover:bg-primary-100 transition-colors">
          العودة للطلبات
        </Link>
      </div>
    )
  ...
  return (
    <div className="pb-12 animate-fade-in">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <InfoCard title="البيانات الشخصية" icon="👤">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
              <InfoRow label="الاسم الكامل" value={pd.full_name} />
              <InfoRow label="الاسم بالأحرف اللاتينية" value={pd.full_name_latin} />
              <InfoRow label="الجنس" value={pd.gender === 'male' ? 'ذكر' : pd.gender === 'female' ? 'أنثى' : pd.gender} />
              <InfoRow label="تاريخ الميلاد" value={pd.date_of_birth ? new Date(pd.date_of_birth).toLocaleDateString('ar-SA') : '-'} />
              <InfoRow label="مكان الولادة" value={pd.birth_city} />
              <InfoRow label="الدولة" value={pd.country} />
              <InfoRow label="الجنسية" value={pd.nationality} />
              <InfoRow label="الأهلية (ثانوية عامة)" value={pd.has_high_school === 'yes' ? 'نعم' : pd.has_high_school === 'no' ? 'لا' : '-'} />
            </div>
          </InfoCard>
          ...
          <InfoCard title="معلومات الاتصال" icon="📞">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
              <InfoRow label="الهاتف" value={pd.phone} direction="ltr" />
              <InfoRow label="البريد الإلكتروني" value={pd.email} />
              <InfoRow label="العنوان" value={pd.address} className="md:col-span-2" />
            </div>
          </InfoCard>
          ...
          <InfoCard title="التعليم والحالة الاجتماعية" icon="🎓">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
              <InfoRow label="المستوى التعليمي" value={pd.education_level} />
              <InfoRow label="الحالة الاجتماعية" value={pd.marital_status} />
            </div>
          </InfoCard>

          {/* Spouse */}
          {(pd.marital_status === 'married' || pd.spouse_data) && (
            <InfoCard title="معلومات الزوج/ة" icon="👫" highlight>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
                <InfoRow label="الاسم" value={pd.spouse_data?.full_name} />
                <InfoRow label="تاريخ الميلاد" value={pd.spouse_data?.date_of_birth ? new Date(pd.spouse_data.date_of_birth).toLocaleDateString('ar-SA') : '-'} />
                <InfoRow label="مكان الولادة" value={pd.spouse_data?.birth_city} />
                <InfoRow label="الجنسية" value={pd.spouse_data?.nationality} />
              </div>
            </InfoCard>
          )}

          {/* Children */}
          {pd.children_data?.length > 0 && (
            <InfoCard title="الأطفال" icon="👶" highlight>
              <div className="space-y-6">
                {pd.children_data.map((child, idx) => (
                  <div key={idx} className={`grid grid-cols-1 md:grid-cols-2 gap-4 ${idx > 0 ? 'border-t border-navy-100 pt-6' : ''}`}>
                    <div className="md:col-span-2 font-bold text-navy-800 text-lg mb-2">الطفل {idx + 1}</div>
                    <InfoRow label="الاسم الكامل" value={child.full_name} />
                    <InfoRow label="تاريخ الميلاد" value={child.date_of_birth ? new Date(child.date_of_birth).toLocaleDateString('ar-SA') : '-'} />
                  </div>
                ))}
              </div>
            </InfoCard>
          ...
          {order.audit_log?.length > 0 && (
            <InfoCard title="سجل الحالة" icon="📜">
                  ...
                  <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                    ...
                    <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white p-4 rounded-2xl border border-navy-100 shadow-sm">
                      ...
                      <p className="text-xs font-semibold text-navy-400 mb-2">
                        ...
                        {log.user?.full_name ? ` • بواسطة ${log.user.full_name}` : ''}
                      </p>
                      {log.notes && (
                        <div className="bg-navy-50 p-3 rounded-lg text-sm text-navy-700 font-medium">
                          {log.notes}
                        </div>
                      )}
                    </div>
        ...
        {/* Sidebar */}
        <div className="space-y-6">
          {/* Actions - Placed top for better UX for employees */}
          <div className="bg-white rounded-3xl shadow-lg shadow-navy-100/50 border border-primary-100 p-6 lg:p-8 sticky top-24">
              ...
              <div key={btn.action} className="mb-4">
                <button
                  ...
                  className={`w-full py-4 rounded-xl font-black mb-3 text-sm transition-all shadow-md hover:-translate-y-1 disabled:opacity-50 disabled:hover:translate-y-0 ${
                    btn.variant === 'primary' ? 'bg-primary-500 text-white shadow-primary-500/30 hover:bg-primary-600 hover:shadow-primary-600/40' :
                    btn.variant === 'success' ? 'bg-emerald-500 text-white shadow-emerald-500/30 hover:bg-emerald-600 hover:shadow-emerald-600/40' :
                    btn.variant === 'error' ? 'bg-error text-white shadow-error/30 hover:bg-red-600 hover:shadow-error/40' :
                    'bg-gold-500 text-white shadow-gold-500/30 hover:bg-gold-600 hover:shadow-gold-600/40'
                  }`}
                >
                  {btn.label}
                </button>
                {btn.requiresReceipt && (
                  <input
                    type="text"
                    placeholder="أدخل معرف الإيصال لتأكيد الدفع..."
                    value={receiptId}
                    onChange={(e) => setReceiptId(e.target.value)}
                    className="w-full bg-navy-50 border border-navy-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:border-primary-300 focus:ring-4 focus:ring-primary-500/10 outline-none transition-all"
                  />
                )}
              </div>
            ))}
            ...
            <div className="mt-6">
              ...
              <textarea
                placeholder="اكتب سبب الرفض أو التعديل هنا ليرد للعميل..."
                value={notes}
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-navy-100 overflow-hidden">
            ...
            {order.photo_validation && (
              <div className="p-6 border-t border-navy-100">
                ...
                <div className="space-y-3">
                  <ValidationResult label="تمركز الوجه" value={order.photo_validation.face_center} />
                  <ValidationResult label="الخلفية البيضاء" value={order.photo_validation.white_background} />
                  <ValidationResult label="توزيع الإضاءة" value={order.photo_validation.lighting} />
                  <ValidationResult label="وضوح الصورة" value={order.photo_validation.clarity} />
                  
                  {order.photo_validation.overall_score && (
                    <div className="mt-4 pt-4 border-t border-navy-100">
                      <div className="flex justify-between items-center mb-2">
                        <span className="font-bold text-navy-700">النتيجة الإجمالية</span>
                        <span className={`font-black text-lg ${order.photo_validation.overall_score >= 80 ? 'text-emerald-500' : order.photo_validation.overall_score >= 60 ? 'text-gold-500' : 'text-error'}`}>
                          {order.photo_validation.overall_score}%
                        </span>
                      </div>
                      <div className="h-2 bg-navy-100 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full ${order.photo_validation.overall_score >= 80 ? 'bg-emerald-500' : order.photo_validation.overall_score >= 60 ? 'bg-gold-500' : 'bg-error'}`}
                          style={{ width: `${order.photo_validation.overall_score}%` }}
                        />
                      </div>
                    </div>
                  )}
            ...
            {payment.id ? (
              <div className="space-y-4">
                <div className="bg-navy-50 rounded-2xl p-4 flex items-center justify-between border border-navy-100">
                  <span className="text-navy-500 font-semibold text-sm">المبلغ المدفوع</span>
                  <span className="text-xl font-black text-success direction-ltr">${payment.amount || '0.00'}</span>
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <span className="block text-xs font-bold text-navy-400 mb-1">طريقة الدفع</span>
                    <span className="font-semibold text-navy-900">{payment.method}</span>
                  </div>
                  <div>
                    <span className="block text-xs font-bold text-navy-400 mb-1">المزود</span>
                    <span className="font-semibold text-navy-900">{payment.provider}</span>
                  </div>
                  <div className="col-span-2">
                    <span className="block text-xs font-bold text-navy-400 mb-1">رقم الحوالة / المعرف</span>
                    <span className="font-semibold text-navy-900 font-mono tracking-wider">{payment.transfer_number || 'غير متوفر'}</span>
                  </div>
                </div>

                {payment.receipt_url && (
                  <button
                    onClick={() => setEnlargeImage(payment.receipt_url)}
              ...
              <div className="flex flex-col items-center justify-center py-6 text-navy-400 bg-navy-50/50 rounded-2xl border border-dashed border-navy-200">
                ...
                <span className="font-semibold text-sm">لا توجد بيانات دفع متاحة</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-navy-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowConfirm(null)}>
          <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full border border-navy-100" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-navy-500 font-medium mb-6">
              هل أنت متأكد من تنفيذ هذا الإجراء: <strong className="text-navy-900 bg-navy-50 px-2 py-1 rounded">{showConfirm.label}</strong>؟
            </p>
            
            {showConfirm.requiresConfirmation && (
              <div className="mb-6">
                <label className="block text-sm font-bold text-navy-700 mb-2">رقم التأكيد (Confirmation Number)</label>
                <input
                  type="text"
                  placeholder="أدخل الرقم الصادر من الموقع الرسمي..."
                ...
                <p className="text-xs text-navy-400 mt-2 flex items-center gap-1">
                  <svg className="w-4 h-4 text-warning" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" /></svg>
                  هذا الإجراء سيبلغ العميل بنجاح التقديم النهائي.
                </p>
              </div>
            )}
